...
printf "${BLUE} Сейчас проверим свободен ли порт 13231 и не установлен ли другой wireguard.\n${NC}"

if [[ $(docker ps -q --filter "name=wireguard") ]]; then
    printf "!!!!>>> Другой Wireguard контейнер уже запущен, и вероятно занимает порт 13231. Пожалуйста удалите его и запустите скрипт заново\n "
    ...
    if lsof -Pi :13231 -sTCP:LISTEN -t >/dev/null ; then
        printf "${RED}!!!!>>> Порт 13231 уже используется контейнером.!\n ${NC}"
        if docker ps --format '{{.Names}} {{.Ports}}' | grep -q "wireguard.*:13231->" ; then
            printf "WireGuard контейнер использует порт 13231. Хотите продолжить установку? (y/n): "
            read -r choice
            case "$choice" in 
